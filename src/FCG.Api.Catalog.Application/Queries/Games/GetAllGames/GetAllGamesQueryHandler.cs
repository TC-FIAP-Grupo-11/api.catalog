...
public class GetAllGamesQueryHandler(IGameRepository gameRepository, IDistributedCache cache)
    : IRequestHandler<GetAllGamesQuery, Result<PagedResult<Game>>>
{
    ...
    {
        var cacheKey = CacheKeys.GamesAll(request.PageNumber, request.PageSize);
        ...
        var pagedGames = await gameRepository.GetAllPagedAsync(request.PageNumber, request.PageSize, cancellationToken);
